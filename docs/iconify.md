...
- `icons.json` — словник `{ "icon-name": { "name": "Назва", "mediaKey": "uuid", "keys": ["ключові", "слова"] } }`. `name` і `keys` формуються на основі майстер-файлів перекладів та англомовних токенів, `mediaKey` — стабільний UUID, що створюється під час імпорту.
- `translations/icons/icon-translations/name-translations.json` — майстер-словник для назв іконок (ключ — базовий slug).
- `translations/tag-translations.json` — спільний майстер-словник для тегів/ключових слів (використовується і для іконок, і для зображень Unsplash).
...
Ключові слова (`keys` у `icons.json`) збираються з таких джерел:
- назви файлів (slug) іконок;
- теги та категорії, визначені в колекції;
- загального словника `translations/tag-translations.json`;
- синонімів із `translations/icons/icon-translations/synonyms.json`.

Назви (`name` у `icons.json`) формуються з `translations/icons/icon-translations/name-translations.json`: для кожного базового slug береться значення з майстер-файлу (якщо переклад не знайдено, тимчасово використовується англомовна людиночитна фраза).
...
- Майстер-файли `translations/icons/icon-translations/name-translations.json`, `translations/tag-translations.json` та `translations/icons/icon-translations/synonyms.json` редагуються вручну й версіонуються в репозиторії. Скрипт лише читає їх.
...
- Під час наступного запуску `pnpm run iconify:pull -- --collection <код>` скрипт одразу підставить оновлені значення у `icons.json`. Токени, які залишилися без перекладу, знову з’являться у відповідних `missing-*` каталогах.
...
1. `pnpm run iconify:pull -- --collection <код>` — завантаження SVG, оновлення `icons.json` і генерація списків відсутніх перекладів/синонімів.
2. За потреби відредагуйте `translations/icons/missing-*` та перенесіть записи до майстер-файлів (`name-translations.json`, `translations/tag-translations.json`, `synonyms.json`).
...
Команда `pnpm run iconify:update-translations` без аргументів проходить по всіх колекціях у `library/iconify` і перегенеровує `icons.json`, використовуючи найсвіжіші дані з `translations/icons/icon-translations`. Опційно можна вказати конкретну колекцію:
...
1. Згенеруйте всі потрібні колекції командою `pnpm run iconify:pull -- --collection <код>` — структура `library/iconify/<slug>` уже міститиме `files/`, `icons.json` і оновлений `collection-meta.json`.
