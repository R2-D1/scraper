...
- `translations/icon-translations/name-translations.json` — майстер-словник для назв іконок (ключ — базовий slug).
- `translations/icon-translations/key-translations.json` — майстер-словник для окремих ключових слів.
- `translations/icon-translations/synonyms.json` — майстер-файл синонімів (масив рядків для кожного токена).
- `translations/missing-translations/names` та `translations/missing-translations/keys` — шардовані списки токенів, для яких не знайдено переклад у майстер-файлах (формат `part-XXXX.json`).
- `translations/missing-synonyms` — шардовані списки токенів без прописаних синонімів у майстер-файлі.
...
- словника `translations/icon-translations/key-translations.json`;
- синонімів із `translations/icon-translations/synonyms.json`.

Назви `icon-names.json` формуються з `translations/icon-translations/name-translations.json`: для кожного базового slug береться значення з майстер-файлу (якщо переклад не знайдено, тимчасово використовується англомовна людиночитна фраза).
...
- Майстер-файли `translations/icon-translations/name-translations.json`, `translations/icon-translations/key-translations.json` та `translations/icon-translations/synonyms.json` редагуються вручну й версіонуються в репозиторії. Скрипт лише читає їх.
- Після кожного запуску створюються каталоги `translations/missing-translations/names`, `translations/missing-translations/keys` та `translations/missing-synonyms` (файли `part-XXXX.json`). Вони містять токени, яких не вистачає у відповідному майстер-файлі.
...
2. За потреби відредагуйте `translations/missing-*` та перенесіть записи до майстер-файлів (`name-translations.json`, `key-translations.json`, `synonyms.json`).
...
Команда `pnpm run iconify:update-translations` без аргументів проходить по всіх колекціях у `library/iconify` і перегенеровує `icon-names.json` та `icon-keys.json`, використовуючи найсвіжіші дані з `translations/icon-translations`. Опційно можна вказати конкретну колекцію:

```bash
pnpm run iconify:update-translations -- --collection tabler
```

Під час виконання також оновлюються каталоги `translations/missing-*`, тому нові або незаповнені токени одразу потрапляють до відповідних шардованих файлів.
